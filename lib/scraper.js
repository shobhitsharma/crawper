 ...
 * @param config.thread {ScraperThread}
 * @param config.onComplete {function}
 */
var Scraper = function(phantom, config) {
...
Scraper.prototype.preprocessPlugins = function(list, standards, exec) {
...
Scraper.prototype.onNewListener = function(event) {
...
Scraper.prototype.onRemoveListener = function(event) {
...
Scraper.prototype.onPageCreated = function(err, page) {
...
Scraper.prototype.onPageSettings = function() {
...
Scraper.prototype.pageReady = function() {
    if (!this.isReady) {
        this.isReady = true;
        if (this.closed) return;
...
Scraper.prototype.close = function(reason) {
    if (!this.closed) {
        this.closed = true;
...
Scraper.prototype.onClosed = function(reason) {
    console.log('[' + this.id + '] done' + (reason ? ': ' + reason : ''));
    this.config.onComplete();
};

Scraper.prototype.crashed = function() {
...
Scraper.prototype.exec = function(fn, cb, order) {
